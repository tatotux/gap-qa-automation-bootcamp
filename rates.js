const apiKey = "63dbe1eaee29089b6be0d33f";

/**
 * Method that resolve the promise
 */
function resolvePromise(currency) {
  return new Promise(resolve => {
    setTimeout(() => {
      axios.get(`https://v6.exchangerate-api.com/v6/${apiKey}/latest/${currency}`)
      .then(function (response) {
        resolve(response.data.conversion_rates);
      }, 2000).catch(function(error) {
        alert(`The follow currency: ${currency} is not supported, ${error}`);
          })
        });
    });
}
/**
 * Method that get a array of full contries
 */
...
/**
 * Method that print all the rates for a currency in a table
 * @currency the currency value to print rates
 */
async function getRates(currency) {
    const result = await resolvePromise(currency);
      ...
      for (const [code, country] of Object.entries(countries)) {
        if (currency == key && code == key) {
          const finalCoin = {
            ...
            Rate : value,
            Country : country }
            console.table(finalCoin);
      }
      if (typeof currency === "undefined") {
        const coins = Object.entries(result);
        console.table(coins);
      }
    }
    }
