// import { printTable } from './node_modules/console-table-printer';

function convert(currency = 'USD') {
    ...
    fetch(`https://v6.exchangerate-api.com/v6/4b104380097e7a8ceccd5da6/latest/${currency}`)
        ...
        .then(data => {
            ...
            if (arguments.length === 0) {
                for (let key in obj) {
                    for (let key2 in countries) {
                        if (key === countries[key2]['Currency Code']) {
                            obj2.push(countries[key2]['Country'])
                            obj2.push(key)
                            obj2.push(obj[key])
                        }
                    }
            ...
            else if (currency in obj)
                console.log(obj[currency])

        }).catch(function (e) {
            console.error('the currency is not supported')
        })
